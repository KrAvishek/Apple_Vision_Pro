...
function files(index) {
  var data = `https://www.apple.com/105/media/us/apple-vision-pro/2023/7e268c13-eb22-493d-a860-f0637bacb569/anim/360/large/0000.jpg
 ...
 `;
  return data.split("\n")[index];
//  isse jo v text mein convert ho jaati hai images kyuki string rehti hai usko kr dete hai images hai
}

const frameCount = 199;
// jitne frame rhenge count krke likh diye hai
const images = [];
const imageSeq = {
  frame: 1,
};
// isse har baar mein images milega aur jab v milega toh push ho jaayega
for (let i = 0; i < frameCount; i++) {
  const img = new Image();
  img.src = files(i);
  images.push(img);
}
// ease means no animation: 
gsap.to(imageSeq, {
  frame: frameCount - 1,
  snap: "frame",
  ease: `none`,
...
// whenever we referesh please start from 1st index
images[1].onload = render;
// first thing is image and other thing written is tool
function render() {
